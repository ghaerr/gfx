...
void    Initialize();
void    Quit(int);
void Change_Screen_Saver( int);
long GetColor( disp    *, char    *, XColor  *);
void Parse_Arguments( int, char **, char **);
void Create_Window( char *);
void Animate();
void Traverse_Tree( Display *, Window);
...
char    **geometry;
{
            ...
            != EOF)
    {
        switch (option)
        {
            ...
            case '?':
                //Usage(*argv);
                //HandleError("The command line parameters were incorrect.", FATAL);
                break;
        }
    }
...
void Bounce_Against_Windows(wx, wy, wxv, wyv, BehindWindow)
int     wx[3], wy[3];
int     *wxv, *wyv;
int     BehindWindow;
{
